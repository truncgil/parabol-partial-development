<?php $backgroundColor = "#fff;"; ?><div class="row">
    <div class="col-100">
        <div class="text">
            <h3>
                {{ $textDocumentTitle }}
                        ...
                        <tr>
                            @stack('name_th_start')
                                @if (!$hideItems || (!$hideName && !$hideDescription))
                                    <th class="item text-left text-white bg-primary">{{ (trans_choice($textItems, 2) != $textItems) ? trans_choice($textItems, 2) : trans($textItems) }}</th>
                                @endif
                            @stack('name_th_end')

                            @stack('quantity_th_start')
                                @if (!$hideQuantity)
                                    <th class="quantity text-white bg-primary">{{ trans($textQuantity) }}</th>
                                @endif
                            @stack('quantity_th_end')

                            @stack('price_th_start')
                                @if (!$hidePrice)
                                    <th class="price text-white bg-primary">{{ trans($textPrice) }}</th>
                                @endif
                            @stack('price_th_end')
                            ...
                            @if (!$hideDiscount)
                                @if (in_array(setting('localisation.discount_location', 'total'), ['item', 'both']))
                                    @stack('discount_td_start')
                                        <th class="discount text-white bg-primary">{{ trans('invoices.discount') }}</th>
                            ...
                            @stack('total_th_start')
                                @if (!$hideAmount)
                                    <th class="total text-white  bg-primary">{{ trans($textAmount) }}</th>
                                @endif
                            @stack('total_th_end')
                        </tr>
                    </thead>
