                                ...
                                @stack('timeline_get_paid_body_start')
                                    ...
                                    <div class="mt-3">
                                        @stack('timeline_get_paid_body_button_pay_start')
                                            @if (!$hideButtonPaid)
                                                @can($permissionUpdate)
                                                <?php if(getisset("mark-paid")) {
                                                    db("documents")
                                                    ->where("id",$document->id)
                                                    ->where("company_id",company_id())
                                                    ->update(['status'=>'paid']);
                                                    yonlendir("/".company_id()."/sales/invoices/".$document->id);
                                                } ?>
                                                    <a onclick="return confirm('Yalnızca Ödendi olarak işaretlenecektir herhangi bir tahsilat yapılmayacaktır. Onaylıyor musunuz?')" href="{{company_id()}}/sales/invoices/{{$document->id}}?mark-paid" class="btn btn-white btn-sm header-button-top">
                                                            Yalnızca Ödendi Olarak İşaretle
                                                        </a>
                                                    <a onclick="return confirm('Fatura tutarı varsayılan hesaba tahsilat olarak geçilip Ödendi olarak işaretlenecektir. Onaylıyor musunuz?')" href="{{ route($routeButtonPaid, $document->id) }}" class="btn btn-white btn-sm header-button-top">
