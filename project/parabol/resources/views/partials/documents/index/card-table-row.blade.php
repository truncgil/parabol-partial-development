    ...
    @if (!$hideAmount)
        <td class="{{ $classAmount }}">
            ...
            if($item->status=="partial") {
               // print2($item);
              // $currencyRate = db("documents")->where("id")
               $yapilanOdemeler = db("transactions")
               //->selectRaw("SUM(amount*currency_rate) AS toplam")
               ->selectRaw("SUM(amount) AS toplam")
               ->whereNull('deleted_at')
               ->where("document_id",$item->id)->first();
               if($item->currency_code != "TRY") {
                    $yapilanOdemeler->toplam = $yapilanOdemeler->toplam / $item->currency_rate;
               }
               
                ?>
                <div class="d-block badge badge-success" title="Yapılan Ödemeler Toplamı">@money($yapilanOdemeler->toplam, $item->currency_code, true)</div>
                <div class="d-block badge badge-warning" title="Kalan Ödeme">@money($item->amount - $yapilanOdemeler->toplam, $item->currency_code, true)</div>
